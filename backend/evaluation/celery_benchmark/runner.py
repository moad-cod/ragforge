"""Celery benchmark runner for durable RAGForge ingestion."""
...
from evaluation.celery_benchmark.client import CeleryBenchmarkClient, BenchmarkIdentity
from evaluation.celery_benchmark.metrics import summarize_runs
from evaluation.celery_benchmark.models import (
    BenchmarkConfig,
    ExperimentReport,
    RunMeasurement,
    WorkloadDocument,
...
from evaluation.celery_benchmark.report import write_json_report, write_markdown_report
from evaluation.celery_benchmark.validator import validate_indexed_run
from evaluation.celery_benchmark.workload import build_default_workload
...
class CeleryBenchmarkRunner:
    def __init__(
        self,
        config: BenchmarkConfig,
        *,
        client: CeleryBenchmarkClient | None = None,
        workload: list[WorkloadDocument] | None = None,
    ) -> None:
        self.config = config
        self.client = client or CeleryBenchmarkClient(config.api_url, timeout_seconds=config.timeout_seconds)
    ...
    async def run(self) -> ExperimentReport:
        experiment_id = f"celery-{uuid.uuid4().hex[:12]}"
        started_at = utc_now()
