"""Benchmark workload construction for the current Celery pipeline."""
...
from evaluation.celery_benchmark.models import WorkloadDocument
...
def deterministic_text(marker: str, *, paragraphs: int = 12) -> bytes:
    paragraph = (
        f"{marker} verifies the RAGForge Celery ingestion path from Bronze storage "
...
def build_default_workload(
    *,
    ...
    return [
        WorkloadDocument(
            document_id=f"{dataset_version}-celery-{index + 1:04d}",
            filename=f"{dataset_version}-celery-{index + 1:04d}.txt",
            content=deterministic_text(f"{dataset_version}_celery_doc_{index + 1:04d}"),
