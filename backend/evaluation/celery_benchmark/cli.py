"""Command-line entry point for the Celery benchmark."""
...
from evaluation.celery_benchmark.models import BenchmarkConfig
from evaluation.celery_benchmark.runner import CeleryBenchmarkRunner
from evaluation.celery_benchmark.workload import load_manifest


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run a RAGForge Celery ingestion benchmark.")
    parser.add_argument("--api-url", default="http://localhost:8000")
    parser.add_argument("--experiment-name", default="celery-controlled-ingestion")
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("artifacts/benchmark-results/celery"))
...
async def async_main() -> int:
    ...
    report = await CeleryBenchmarkRunner(config, workload=workload).run()
