...
CHUNKER_REGISTRY: dict[str, ChunkerDefinition] = {
    ...
    "sentence": ChunkerDefinition(
        ...
        not_recommended_for=["extremely large files where speed is the top priority", "text with unconventional punctuation where sentence boundaries are ambiguous"],
        speed_level="medium",
        quality_level="high",
        cost_level="low",
        requires_llm=False,
    ...
    "hierarchical": ChunkerDefinition(
        ...
        status="beta",
        is_beta=True,
        short_description="Parent-and-child sentence grouping for broad context and precise retrieval units.",
        long_description="Structured Chunking creates deterministic parent groups and links each smaller child group to its parent. It preserves a synthetic sentence hierarchy so retrieval can use precise child units while retaining broader parent context. It does not infer headings, sections, or layout from plain text.",
        best_for=["manuals", "policy documents", "technical documentation", "long reports", "enterprise knowledge bases", "retrieval requiring parent context"],
        not_recommended_for=["short documents", "workflows requiring heading or page hierarchy", "messy OCR text with unreliable sentence boundaries"],
        speed_level="medium",
        quality_level="high",
        cost_level="medium",
        requires_llm=False,
    ...
    "late_chunking": ChunkerDefinition(
        id="late_chunking",
        name="Pooled Sentence Chunking",
        tier="ultimate",
        status="beta",
        is_beta=True,
        short_description="Sentence-group chunking with normalized mean-pooled dense vectors.",
        long_description="Pooled Sentence Chunking groups adjacent sentences and creates each chunk vector by mean-pooling their independently generated sentence embeddings. The pipeline reuses these vectors during indexing to avoid a second embedding pass. This is embedding-aware chunk construction, but it is not token-level late chunking over a single full-document contextual encoding.",
        best_for=["evaluation-focused experiments", "technical content", "dense documents", "workflows that reuse precomputed dense vectors", "sentence-group retrieval"],
        not_recommended_for=["workflows requiring true token-level late chunking", "simple documents where paragraph chunking is enough", "lightweight environments without an embedding model"],
        speed_level="medium",
        quality_level="high",
        cost_level="medium",
        requires_llm=False,
