...
export function RetrievalTrace({items, onSelect}: {items: RetrievalTraceType[]; onSelect?: (item: RetrievalTraceType) => void}) {
  ...
  return <div className="space-y-3">{items.map((item) => <article key={item.retrieval_log_id} className="rounded-xl border border-white/[0.08] bg-[#0a1511] p-4">
    ...
    {onSelect ? <button onClick={() => onSelect(item)} className="mt-3 text-[10px] font-medium text-emerald-300 hover:text-emerald-200">Open supporting source →</button> : null}
  </article>)}</div>;
}
