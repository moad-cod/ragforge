...
export function ProjectForm({organizations, chunkers, isPending, submitLabel = "Create project", initialName = "", onCancel, onSubmit}: {
  ...
  return <form className="mt-6 space-y-4" onSubmit={handleSubmit(onSubmit)}>
    ...
    {chunkers ? <label className="block"><span className="mb-2 block text-xs font-medium">Default chunking strategy</span><select className="h-10 w-full rounded-lg border border-[var(--border)] bg-[var(--surface-muted)] px-3 text-sm outline-none focus:border-[var(--accent)]" {...register("chunker")}>{chunkers.filter((item) => item.id !== "multimodal").map((item) => <option key={item.id} value={item.id}>{item.name}{item.default ? " · Recommended" : ""}</option>)}</select><span className="mt-1.5 block text-[10px] leading-4 text-[#64736d]">Saved as a browser preference and applied to the upload form. The backend stores the chosen strategy on each document version.</span></label> : null}
    <label className="block"><span className="mb-2 block text-xs font-medium">Description <span className="font-normal text-[#64736d]">(not supported by the current API)</span></span><textarea disabled className="min-h-20 w-full resize-none rounded-lg border border-white/[0.06] bg-white/[0.015] px-3 py-2.5 text-sm text-[#53625b]" placeholder="Project descriptions will be available when backend support is added." /></label>
    <details className="rounded-lg border border-white/[0.08] bg-white/[0.018] p-3"><summary className="cursor-pointer text-xs font-medium">Advanced configuration</summary><dl className="mt-3 grid gap-3 text-[10px] sm:grid-cols-2"><div><dt className="text-[#64736d]">Embedding model</dt><dd className="mono mt-1 text-[#b8c5bf]">BAAI/bge-small-en-v1.5</dd></div><div><dt className="text-[#64736d]">Retrieval</dt><dd className="mt-1 text-[#b8c5bf]">Hybrid dense + sparse</dd></div></dl><p className="mt-3 text-[9px] leading-4 text-[#53625b]">These values reflect the backend defaults and are read-only because project-level configuration is not currently exposed.</p></details>
