...
function OnboardingProgress({step}: {step: OnboardingStep}) {
  ...
  return <div className="mx-auto flex w-full max-w-3xl items-start">
    {progressSteps.map((label, index) => <div key={label} className="flex flex-1 items-start last:flex-none">
      <div className="flex w-24 flex-col items-center text-center sm:w-36"><span className={cn("flex size-7 items-center justify-center rounded-full border text-[10px] font-semibold", index < active ? "border-emerald-400 bg-emerald-400 text-[#052116]" : index === active ? "border-emerald-400 bg-emerald-400/10 text-emerald-300" : "border-white/10 bg-white/[0.025] text-[#53625b]")}>{index < active ? <Check className="size-3.5" /> : index + 1}</span><span className={cn("mt-2 hidden text-[9px] sm:block", index <= active ? "text-[#c4d1cb]" : "text-[#53625b]")}>{label}</span></div>
      {index < progressSteps.length - 1 ? <span className={cn("mt-3.5 h-px min-w-4 flex-1", index < active ? "bg-emerald-400/60" : "bg-white/10")} /> : null}
    </div>)}
  </div>;
}
...
function ProcessingRun({initialRun, filename, onStatus}: {initialRun: IngestionRun; filename: string; onStatus: (run: IngestionRun) => void}) {
  ...
  return <section className="rounded-xl border border-white/[0.09] bg-[#0a1511]">
    <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/[0.08] px-4 py-3">
      ...
      {!failed && run.status !== "indexed" ? <span className={cn("flex items-center gap-1.5 rounded-full border px-2 py-1 text-[8px]", connected ? "border-emerald-400/20 bg-emerald-400/[0.07] text-emerald-300" : "border-amber-400/20 bg-amber-400/[0.07] text-amber-300")}><span className={cn("size-1.5 rounded-full", connected ? "bg-emerald-400" : "bg-amber-400")} />{connected ? "Live pipeline updates" : "Reconnecting to pipeline…"}</span> : null}
    </div>
    <div className="p-4 sm:p-5">
      {failed ? <div className="rounded-lg border border-red-400/20 bg-red-400/[0.055] p-4"><div className="flex gap-3"><AlertTriangle className="mt-0.5 size-5 shrink-0 text-red-300" /><div><h4 className="text-[11px] font-semibold text-red-200">We could not complete document indexing.</h4><p className="mt-1 text-[9px] leading-4 text-red-200/70">Your uploaded file is safe and you can retry the failed stage.</p><p className="mt-3 rounded-md bg-black/15 p-2 font-mono text-[8px] leading-4 text-red-200/80">{run.error_message || "The ingestion pipeline stopped unexpectedly."}</p></div></div></div> : <div className="space-y-0">{pipelineStages.map((stage, index) => {
        ...
        return <div key={stage.name} className="flex gap-3"><div className="flex flex-col items-center"><span className={cn("flex size-6 shrink-0 items-center justify-center rounded-full border", complete ? "border-emerald-400 bg-emerald-400 text-[#052116]" : active ? "border-blue-400 bg-blue-400/10 text-blue-300" : "border-white/10 bg-white/[0.02] text-[#40524a]")}>{complete ? <Check className="size-3" /> : active ? <LoaderCircle className="size-3 animate-spin" /> : <Circle className="size-2.5" />}</span>{index < pipelineStages.length - 1 ? <span className={cn("h-10 w-px", complete ? "bg-emerald-400/50" : "bg-white/[0.08]")} /> : null}</div><div className="min-w-0 flex-1 pb-4"><div className="flex items-center justify-between gap-2"><h4 className={cn("text-[10px] font-medium", complete || active ? "text-[#dce6e1]" : "text-[#64736d]")}>{stage.name}</h4><span className="text-[8px] capitalize text-[#53625b]">{complete ? "Completed" : active ? "Running" : "Waiting"}</span></div><p className="mt-0.5 text-[8px] leading-4 text-[#53625b]">{stage.description}</p></div></div>;
      })}</div>}
...
export function ProjectOnboarding({projectId}: {projectId: string}) {
  ...
  return <div className="h-full overflow-y-auto bg-[#07110d]">
    <div className="mx-auto min-h-full max-w-5xl px-4 py-6 sm:px-7 sm:py-8">
      ...
      {effectiveStep === "knowledge" ? <section className="mx-auto mt-10 max-w-3xl">
        <div className="text-center"><p className="text-[10px] font-semibold uppercase tracking-[.15em] text-emerald-300">Step 2 of 4</p><h1 className="mt-2 text-2xl font-semibold">Add knowledge to your project</h1><p className="mx-auto mt-2 max-w-xl text-[12px] leading-5 text-[#71847b]">Upload at least one document to prepare your RAGForge knowledge base. You can add more sources later from the workspace.</p></div>
        <div className="mt-7 rounded-xl border border-white/[0.09] bg-[#0a1511] p-3 sm:p-5">
          <div className="grid grid-cols-3 gap-1 rounded-lg bg-white/[0.025] p-1">{([{id:"file",label:"Upload files",icon:UploadCloud},{id:"url",label:"Add a URL",icon:Globe2},{id:"gdrive",label:"Google Drive",icon:Cloud}] as const).map(({id,label,icon:Icon}) => <button key={id} onClick={() => setMode(id)} className={cn("flex h-9 items-center justify-center gap-1.5 rounded-md text-[9px]", mode === id ? "bg-[#173027] text-emerald-200" : "text-[#64736d] hover:text-[#a9b7b0]")}><Icon className="size-3" />{label}</button>)}</div>
          {mode === "file" ? <button onClick={() => inputRef.current?.click()} onDragOver={(event) => event.preventDefault()} onDrop={(event) => {event.preventDefault(); addFiles(event.dataTransfer.files);}} className="mt-3 flex min-h-48 w-full flex-col items-center justify-center rounded-xl border border-dashed border-white/[0.16] bg-white/[0.015] px-5 text-center transition hover:border-emerald-400/40 hover:bg-emerald-400/[0.025]"><span className="flex size-11 items-center justify-center rounded-xl bg-emerald-400/10 text-emerald-300"><HardDriveUpload className="size-5" /></span><strong className="mt-4 text-[12px] font-medium">Drop your files here or browse your computer</strong><span className="mt-1.5 text-[9px] text-[#64736d]">PDF, DOCX, PPTX, XLSX, CSV, TXT and HTML · Maximum 25 MB per file</span></button> : mode === "url" ? <div className="mt-3 rounded-xl border border-white/[0.09] p-5"><label className="text-[10px] font-medium">Public webpage URL</label><div className="mt-2 flex gap-2"><input value={url} onChange={(event) => setUrl(event.target.value)} placeholder="https://example.com/research" className="h-10 min-w-0 flex-1 rounded-lg border border-white/10 bg-white/[0.025] px-3 text-[10px] outline-none focus:border-emerald-400/40" /><button onClick={addRemoteSource} className="rounded-lg bg-emerald-400 px-4 text-[9px] font-semibold text-[#052116]">Add URL</button></div></div> : <div className="mt-3 space-y-3 rounded-xl border border-white/[0.09] p-5"><label className="block text-[10px] font-medium">Google Drive file ID<input value={driveId} onChange={(event) => setDriveId(event.target.value)} className="mt-2 h-10 w-full rounded-lg border border-white/10 bg-white/[0.025] px-3 text-[10px] outline-none focus:border-emerald-400/40" /></label><label className="block text-[10px] font-medium">Access token<input type="password" value={driveToken} onChange={(event) => setDriveToken(event.target.value)} className="mt-2 h-10 w-full rounded-lg border border-white/10 bg-white/[0.025] px-3 text-[10px] outline-none focus:border-emerald-400/40" /></label><button onClick={addRemoteSource} className="h-9 rounded-lg bg-emerald-400 px-4 text-[9px] font-semibold text-[#052116]">Add Google Drive file</button></div>}
          ...
          {sources.length ? <div className="mt-4"><p className="mb-2 text-[9px] font-medium text-[#93a39c]">Ready to add ({sources.length})</p><div className="space-y-1.5">{sources.map((source) => <div key={source.id} className="flex items-center gap-3 rounded-lg border border-white/[0.08] bg-white/[0.02] px-3 py-2.5"><FileText className="size-4 text-emerald-300" /><div className="min-w-0 flex-1"><p className="truncate text-[10px] font-medium">{source.name}</p><p className="truncate text-[8px] text-[#53625b]">{source.kind.toUpperCase()} · {source.detail}</p></div><span className="rounded-full bg-emerald-400/10 px-2 py-1 text-[7px] text-emerald-300">Valid</span><button onClick={() => setSources((current) => current.filter((item) => item.id !== source.id))} className="text-[#53625b] hover:text-white"><X className="size-3" /></button></div>)}</div></div> : null}
        </div>
        <div className="mt-5 flex flex-wrap items-center justify-between gap-3"><button onClick={() => router.push("/projects")} className="text-[9px] text-[#71847b] hover:text-white">Skip for now <span className="text-[#53625b]">· Chat remains unavailable until a document is indexed</span></button><button disabled={!validSources} onClick={() => setStep("configure")} className="flex h-10 items-center gap-2 rounded-lg bg-emerald-400 px-5 text-[10px] font-semibold text-[#052116] hover:bg-emerald-300 disabled:bg-white/[0.06] disabled:text-[#53625b]">Continue<ArrowRight className="size-3.5" /></button></div>
      </section> : null}

      {effectiveStep === "configure" ? <section className="mx-auto mt-10 max-w-3xl"><div className="text-center"><p className="text-[10px] font-semibold uppercase tracking-[.15em] text-emerald-300">Step 3 of 4</p><h1 className="mt-2 text-2xl font-semibold">Prepare your documents for retrieval</h1><p className="mx-auto mt-2 max-w-xl text-[12px] leading-5 text-[#71847b]">Choose how RAGForge should parse, chunk and index your documents. Recommended settings work well for most projects.</p></div>
        <div className="mt-7 grid gap-4 md:grid-cols-[0.8fr_1.2fr]"><div className="rounded-xl border border-white/[0.09] bg-[#0a1511] p-4"><h3 className="text-[10px] font-semibold">Knowledge sources</h3><p className="mt-1 text-[8px] text-[#64736d]">{sources.length} {sources.length === 1 ? "source" : "sources"} ready</p><div className="mt-3 space-y-2">{sources.map((source) => <div key={source.id} className="flex items-center gap-2 rounded-lg bg-white/[0.025] p-2.5"><FileText className="size-3.5 text-emerald-300" /><span className="min-w-0 flex-1 truncate text-[9px]">{source.name}</span><ShieldCheck className="size-3 text-emerald-300" /></div>)}</div></div>
          <div className="rounded-xl border border-white/[0.09] bg-[#0a1511] p-4"><label className="block"><span className="text-[9px] font-medium">Chunking strategy</span><select value={chunkerId} onChange={(event) => setChunkerId(event.target.value)} className="mt-2 h-10 w-full rounded-lg border border-white/10 bg-[#0e1c17] px-3 text-[10px] outline-none focus:border-emerald-400/40">{chunkers.map((chunker) => <option key={chunker.id} value={chunker.id}>{chunker.name}{chunker.default ? " · Recommended" : ""}</option>)}</select><p className="mt-1.5 text-[8px] leading-4 text-[#64736d]">{chunkers.find((chunker) => chunker.id === chunkerId)?.short_description ?? "Fast, reliable splitting for general documents and reports."}</p></label><div className="mt-4 grid gap-3 sm:grid-cols-2"><label className="text-[9px] font-medium">Embedding model<select value={embeddingModel} onChange={(event) => setEmbeddingModel(event.target.value)} className="mt-2 h-10 w-full rounded-lg border border-white/10 bg-[#0e1c17] px-3 text-[9px] outline-none"><option>BAAI/bge-small-en-v1.5</option><option>text-embedding-3-small</option></select></label><label className="text-[9px] font-medium">Retrieval strategy<select value={retrievalStrategy} onChange={(event) => setRetrievalStrategy(event.target.value)} className="mt-2 h-10 w-full rounded-lg border border-white/10 bg-[#0e1c17] px-3 text-[9px] outline-none"><option value="hybrid">Hybrid search · Recommended</option><option value="dense">Semantic search</option><option value="sparse">Keyword search</option></select></label></div><button onClick={() => setAdvanced((value) => !value)} className="mt-4 flex w-full items-center justify-between border-t border-white/[0.07] pt-3 text-[9px] text-[#71847b]"><span className="flex items-center gap-1.5"><Settings2 className="size-3" />Advanced ingestion options</span><ChevronDown className={cn("size-3 transition", advanced && "rotate-180")} /></button>{advanced ? <div className="mt-3 grid grid-cols-2 gap-2 rounded-lg bg-white/[0.02] p-3 text-[8px] text-[#64736d]"><label className="flex items-center gap-2"><input type="checkbox" defaultChecked className="accent-emerald-400" />Preserve page metadata</label><label className="flex items-center gap-2"><input type="checkbox" defaultChecked className="accent-emerald-400" />Enable OCR fallback</label></div> : null}</div></div>
        <div className="mt-5 flex items-center justify-between"><button onClick={() => setStep("knowledge")} className="flex items-center gap-1.5 text-[9px] text-[#71847b] hover:text-white"><ArrowLeft className="size-3" />Back</button><button disabled={createKnowledgeBase.isPending} onClick={() => createKnowledgeBase.mutate()} className="flex h-10 items-center gap-2 rounded-lg bg-emerald-400 px-5 text-[10px] font-semibold text-[#052116] hover:bg-emerald-300 disabled:opacity-50">{createKnowledgeBase.isPending ? <LoaderCircle className="size-3.5 animate-spin" /> : <Database className="size-3.5" />}Create knowledge base</button></div>
      </section> : null}

      {effectiveStep === "processing" ? <section className="mx-auto mt-10 max-w-3xl"><div className="text-center"><p className="text-[10px] font-semibold uppercase tracking-[.15em] text-emerald-300">Step 4 of 4</p><h1 className="mt-2 text-2xl font-semibold">Preparing your knowledge base</h1><p className="mx-auto mt-2 max-w-xl text-[12px] leading-5 text-[#71847b]">RAGForge is processing your documents. Follow each stage in real time.</p></div><div className="mt-7 space-y-3">{createKnowledgeBase.isPending && !effectiveRuns.length ? <div className="flex items-center gap-3 rounded-xl border border-white/[0.09] bg-[#0a1511] p-5"><span className="flex size-9 items-center justify-center rounded-lg bg-blue-400/10 text-blue-300"><LoaderCircle className="size-4 animate-spin" /></span><div><h3 className="text-[11px] font-medium">Uploading and creating pipeline runs</h3><p className="mt-1 text-[8px] text-[#64736d]">Your sources are being validated and transferred to secure Bronze storage.</p></div></div> : null}{effectiveRuns.map((run) => <ProcessingRun key={`${run.ingestion_run_id}:${run.status}`} initialRun={run} filename={sourceNameByDocument.get(run.document_id) ?? sources.find((source) => source.kind === "file")?.name ?? "Uploaded document"} onStatus={handleRunStatus} />)}</div>{failedRun ? <div className="mt-5 flex flex-wrap items-center justify-between gap-3"><button onClick={() => router.push("/projects")} className="text-[9px] text-[#71847b] hover:text-white">Back to project</button><div className="flex gap-2"><details className="relative"><summary className="flex h-9 cursor-pointer list-none items-center rounded-lg border border-white/10 px-3 text-[9px] text-[#93a39c]">Technical details</summary><div className="absolute bottom-11 right-0 z-20 w-80 rounded-lg border border-white/10 bg-[#13251e] p-3 font-mono text-[8px] leading-4 text-[#93a39c] shadow-xl">Run: {failedRun.ingestion_run_id}<br />Status: {failedRun.status}<br />{failedRun.error_message}</div></details><button disabled={retry.isPending} onClick={() => retry.mutate(failedRun.ingestion_run_id)} className="flex h-9 items-center gap-1.5 rounded-lg bg-emerald-400 px-4 text-[9px] font-semibold text-[#052116]"><RefreshCw className={cn("size-3", retry.isPending && "animate-spin")} />Retry failed stage</button></div></div> : null}</section> : null}

      {effectiveStep === "success" ? <section className="mx-auto mt-14 max-w-2xl text-center"><span className="mx-auto flex size-14 items-center justify-center rounded-2xl border border-emerald-400/25 bg-emerald-400/10 text-emerald-300"><CheckCircle2 className="size-7" /></span><p className="mt-5 text-[10px] font-semibold uppercase tracking-[.15em] text-emerald-300">Processing complete</p><h1 className="mt-2 text-2xl font-semibold">Your knowledge base is ready</h1><p className="mx-auto mt-3 max-w-lg text-[12px] leading-6 text-[#71847b]">{indexedDocuments.length || successCount || 1} {(indexedDocuments.length || successCount || 1) === 1 ? "document was" : "documents were"} successfully processed{successChunks ? ` into ${successChunks} searchable chunks` : " and can now be searched"}. You can now ask questions and verify every answer against its original source.</p><button onClick={() => router.push(`/projects/${projectId}/documents`)} className="mx-auto mt-7 flex h-11 items-center gap-2 rounded-lg bg-emerald-400 px-6 text-[10px] font-semibold text-[#052116] hover:bg-emerald-300"><Sparkles className="size-4" />Open AI Workspace<ArrowRight className="size-3.5" /></button></section> : null}
    </div>
  </div>;
}
