...
const progressSteps = ["Project details", "Add sources", "Configure ingestion", "Process sources"];
...
function OnboardingProgress({step}: {step: OnboardingStep}) {
  ...
  return <div className="mx-auto flex w-full max-w-3xl items-start">
    {progressSteps.map((label, index) => <div key={label} className="flex flex-1 items-start last:flex-none">
      <div className="flex w-24 flex-col items-center text-center sm:w-36"><span className={cn("flex size-7 items-center justify-center rounded-full border text-[10px] font-semibold", index < active ? "border-[var(--accent)] bg-[var(--accent)] text-[var(--ink-inverse)]" : index === active ? "border-[var(--accent)] bg-[var(--accent-soft)] text-[var(--accent)]" : "border-white/10 bg-white/[0.025] text-[#5f5952]")}>{index < active ? <Check className="size-3.5" /> : index + 1}</span><span className={cn("mt-2 hidden text-[9px] sm:block", index <= active ? "text-[#c9c1b7]" : "text-[#5f5952]")}>{label}</span></div>
...
function ProcessingRun({initialRun, filename, onStatus}: {initialRun: IngestionRun; filename: string; onStatus: (run: IngestionRun) => void}) {
  ...
  return <section className="rounded-xl border border-white/[0.09] bg-[var(--surface)]">
    <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/[0.08] px-4 py-3">
      <div className="flex min-w-0 items-center gap-3"><span className="flex size-8 items-center justify-center rounded-lg bg-red-400/10 text-red-300"><FileText className="size-4" /></span><div className="min-w-0"><h3 className="truncate text-[12px] font-medium">{filename}</h3><p className="mt-0.5 font-mono text-[8px] text-[#5f5952]">{run.ingestion_run_id}</p></div></div>
    ...
    <div className="p-4 sm:p-5">
      {failed ? <div className="rounded-lg border border-red-400/20 bg-red-400/[0.055] p-4"><div className="flex gap-3"><AlertTriangle className="mt-0.5 size-5 shrink-0 text-red-300" /><div><h4 className="text-[11px] font-semibold text-red-200">We could not complete document indexing.</h4><p className="mt-1 text-[9px] leading-4 text-red-200/70">Your uploaded file is safe and you can retry the failed stage.</p><p className="mt-3 rounded-md bg-black/15 p-2 font-mono text-[8px] leading-4 text-red-200/80">{run.error_message || "The ingestion pipeline stopped unexpectedly."}</p></div></div></div> : <div className="space-y-0">{pipelineStages.map((stage, index) => {
        ...
        const complete = run.status === "indexed" || (index === 1 ? run.progress.bronze : rank > stageRank);
        const active = run.status !== "indexed" && rank === stageRank && !complete;
        const queuedNext = (run.status === "landed" || run.status === "queued") && index === 2;
        const statusLabel = complete ? "Completed" : active ? "Running" : queuedNext ? "Queued" : "Waiting";
        return <div key={stage.name} className="flex gap-3"><div className="flex flex-col items-center"><span className={cn("flex size-6 shrink-0 items-center justify-center rounded-full border", complete ? "border-[var(--accent)] bg-[var(--accent)] text-[var(--ink-inverse)]" : active ? "border-blue-400 bg-blue-400/10 text-blue-300" : "border-white/10 bg-white/[0.02] text-[#403c36]")}>{complete ? <Check className="size-3" /> : active ? <LoaderCircle className="size-3 animate-spin" /> : <Circle className="size-2.5" />}</span>{index < pipelineStages.length - 1 ? <span className={cn("h-10 w-px", complete ? "bg-[var(--accent-muted)]" : "bg-white/[0.08]")} /> : null}</div><div className="min-w-0 flex-1 pb-4"><div className="flex items-center justify-between gap-2"><h4 className={cn("text-[10px] font-medium", complete || active ? "text-[#f4efe7]" : "text-[#77716a]")}>{stage.name}</h4><span className="text-[8px] capitalize text-[#5f5952]">{statusLabel}</span></div><p className="mt-0.5 text-[8px] leading-4 text-[#5f5952]">{stage.description}</p></div></div>;
      })}</div>}
      <div className="mt-1 flex items-center justify-between border-t border-white/[0.07] pt-3 text-[8px] text-[#5f5952]"><span>Elapsed {elapsed < 60 ? `${elapsed}s` : `${Math.floor(elapsed / 60)}m ${elapsed % 60}s`}</span><span>{run.status === "indexed" ? "100%" : `${Math.min(90, Math.max(10, rank * 18))}%`}</span></div>
    </div>
  </section>;
}
    ...
    <div className="mx-auto min-h-full max-w-5xl px-4 py-6 sm:px-7 sm:py-8">
      ...
      {effectiveStep === "knowledge" ? <section className="mx-auto mt-10 max-w-3xl">
        ...
        <div className="mt-7 rounded-xl border border-white/[0.09] bg-[var(--surface)] p-3 sm:p-5">
          ...
          {sources.length ? <div className="mt-4"><p className="mb-2 text-[9px] font-medium text-[#aaa39a]">Ready to add ({sources.length})</p><div className="space-y-1.5">{sources.map((source) => <div key={source.id} className="flex items-center gap-3 rounded-lg border border-white/[0.08] bg-white/[0.02] px-3 py-2.5"><FileText className="size-4 text-[var(--accent)]" /><div className="min-w-0 flex-1"><p className="truncate text-[10px] font-medium">{source.name}</p><p className="truncate text-[8px] text-[#5f5952]">{source.kind.toUpperCase()} · {source.detail}</p></div><span className="rounded-full bg-[var(--accent-soft)] px-2 py-1 text-[7px] text-[var(--accent)]">Valid</span><button onClick={() => setSources((current) => current.filter((item) => item.id !== source.id))} className="text-[#5f5952] hover:text-white"><X className="size-3" /></button></div>)}</div></div> : null}
        </div>
        <div className="mt-5 flex flex-wrap items-center justify-between gap-3"><button onClick={() => router.push("/projects")} className="text-[9px] text-[#8f877f] hover:text-white">Skip for now <span className="text-[#5f5952]">· Chat remains unavailable until a document is indexed</span></button><button disabled={!validSources} onClick={() => setStep("configure")} className="flex h-10 items-center gap-2 rounded-lg bg-[var(--accent)] px-5 text-[10px] font-semibold text-[var(--ink-inverse)] hover:bg-[var(--accent-hover)] disabled:bg-white/[0.06] disabled:text-[#5f5952]">Continue<ArrowRight className="size-3.5" /></button></div>
      </section> : null}

      {effectiveStep === "configure" ? <section className="mx-auto mt-10 max-w-3xl"><div className="text-center"><p className="text-[10px] font-semibold uppercase tracking-[.15em] text-[var(--accent)]">Step 3 of 4</p><h1 className="mt-2 text-2xl font-semibold">Prepare your sources for retrieval</h1><p className="mx-auto mt-2 max-w-xl text-[12px] leading-5 text-[#8f877f]">Choose how RAGForge should parse, chunk and index your sources. Recommended settings work well for most projects.</p></div>
        <div className="mt-7 grid gap-4 md:grid-cols-[0.8fr_1.2fr]"><div className="rounded-xl border border-white/[0.09] bg-[var(--surface)] p-4"><h3 className="text-[10px] font-semibold">Knowledge sources</h3><p className="mt-1 text-[8px] text-[#77716a]">{sources.length} {sources.length === 1 ? "source" : "sources"} ready</p><div className="mt-3 space-y-2">{sources.map((source) => <div key={source.id} className="flex items-center gap-2 rounded-lg bg-white/[0.025] p-2.5"><FileText className="size-3.5 text-[var(--accent)]" /><span className="min-w-0 flex-1 truncate text-[9px]">{source.name}</span><ShieldCheck className="size-3 text-[var(--accent)]" /></div>)}</div></div>
          <div className="rounded-xl border border-white/[0.09] bg-[var(--surface)] p-4"><label className="block"><span className="text-[9px] font-medium">Chunking strategy</span><select value={chunkerId} onChange={(event) => setChunkerId(event.target.value)} className="mt-2 h-10 w-full rounded-lg border border-white/10 bg-[var(--surface-muted)] px-3 text-[10px] outline-none focus:border-[var(--border-hover)]">{chunkers.filter((chunker) => chunker.id !== "multimodal").map((chunker) => <option key={chunker.id} value={chunker.id}>{chunker.name}{chunker.default ? " · Recommended" : ""}</option>)}</select><p className="mt-1.5 text-[8px] leading-4 text-[#77716a]">{chunkers.find((chunker) => chunker.id === chunkerId)?.short_description ?? "Fast, reliable splitting for general documents and reports."}</p></label><div className="mt-4 grid gap-3 sm:grid-cols-2"><div className="rounded-lg border border-white/[0.07] bg-white/[0.018] p-3"><span className="text-[8px] text-[#77716a]">Embedding model</span><p className="mono mt-1 text-[9px]">BAAI/bge-small-en-v1.5</p></div><div className="rounded-lg border border-white/[0.07] bg-white/[0.018] p-3"><span className="text-[8px] text-[#77716a]">Retrieval strategy</span><p className="mt-1 text-[9px]">Hybrid dense + sparse</p></div></div><button onClick={() => setAdvanced((value) => !value)} className="mt-4 flex w-full items-center justify-between border-t border-white/[0.07] pt-3 text-[9px] text-[#8f877f]"><span className="flex items-center gap-1.5"><Settings2 className="size-3" />Advanced ingestion information</span><ChevronDown className={cn("size-3 transition", advanced && "rotate-180")} /></button>{advanced ? <p className="mt-3 rounded-lg bg-white/[0.02] p-3 text-[8px] leading-4 text-[#77716a]">Embedding and retrieval configuration are backend defaults. The current ingestion API accepts the selected chunker but does not expose project-level embedding, OCR, or retrieval configuration.</p> : null}</div></div>
        <div className="mt-5 flex items-center justify-between"><button onClick={() => setStep("knowledge")} className="flex items-center gap-1.5 text-[9px] text-[#8f877f] hover:text-white"><ArrowLeft className="size-3" />Back</button><button disabled={createKnowledgeBase.isPending} onClick={() => createKnowledgeBase.mutate()} className="flex h-10 items-center gap-2 rounded-lg bg-[var(--accent)] px-5 text-[10px] font-semibold text-[var(--ink-inverse)] hover:bg-[var(--accent-hover)] disabled:opacity-50">{createKnowledgeBase.isPending ? <LoaderCircle className="size-3.5 animate-spin" /> : <Database className="size-3.5" />}Create knowledge base</button></div>
      </section> : null}

      {effectiveStep === "processing" ? <section className="mx-auto mt-10 max-w-3xl"><div className="text-center"><p className="text-[10px] font-semibold uppercase tracking-[.15em] text-[var(--accent)]">Step 4 of 4</p><h1 className="mt-2 text-2xl font-semibold">Preparing your sources</h1><p className="mx-auto mt-2 max-w-xl text-[12px] leading-5 text-[#8f877f]">RAGForge is processing your sources. Follow each stage in real time.</p></div><div className="mt-7 space-y-3">{createKnowledgeBase.isPending && !effectiveRuns.length ? <div className="flex items-center gap-3 rounded-xl border border-white/[0.09] bg-[var(--surface)] p-5"><span className="flex size-9 items-center justify-center rounded-lg bg-blue-400/10 text-blue-300"><LoaderCircle className="size-4 animate-spin" /></span><div><h3 className="text-[11px] font-medium">Uploading and creating pipeline runs</h3><p className="mt-1 text-[8px] text-[#77716a]">Your sources are being validated and transferred to secure Bronze storage.</p></div></div> : null}{effectiveRuns.map((run) => <ProcessingRun key={`${run.ingestion_run_id}:${run.status}`} initialRun={run} filename={sourceNameByDocument.get(run.document_id) ?? sources.find((source) => source.kind === "file")?.name ?? "Uploaded source"} onStatus={handleRunStatus} />)}</div>{failedRun ? <div className="mt-5 flex flex-wrap items-center justify-between gap-3"><button onClick={() => router.push("/projects")} className="text-[9px] text-[#8f877f] hover:text-white">Back to project</button><div className="flex gap-2"><details className="relative"><summary className="flex h-9 cursor-pointer list-none items-center rounded-lg border border-white/10 px-3 text-[9px] text-[#aaa39a]">Technical details</summary><div className="absolute bottom-11 right-0 z-20 w-80 rounded-lg border border-white/10 bg-[var(--surface-raised)] p-3 font-mono text-[8px] leading-4 text-[#aaa39a] shadow-xl">Run: {failedRun.ingestion_run_id}<br />Status: {failedRun.status}<br />{failedRun.error_message}</div></details><button disabled={retry.isPending} onClick={() => retry.mutate(failedRun.ingestion_run_id)} className="flex h-9 items-center gap-1.5 rounded-lg bg-[var(--accent)] px-4 text-[9px] font-semibold text-[var(--ink-inverse)]"><RefreshCw className={cn("size-3", retry.isPending && "animate-spin")} />Retry failed stage</button></div></div> : null}</section> : null}

      {effectiveStep === "success" ? <section className="mx-auto mt-14 max-w-2xl text-center"><span className="mx-auto flex size-14 items-center justify-center rounded-2xl border border-[var(--accent-border)] bg-[var(--accent-soft)] text-[var(--accent)]"><CheckCircle2 className="size-7" /></span><p className="mt-5 text-[10px] font-semibold uppercase tracking-[.15em] text-[var(--accent)]">Processing complete</p><h1 className="mt-2 text-2xl font-semibold">Your sources are ready</h1><p className="mx-auto mt-3 max-w-lg text-[12px] leading-6 text-[#8f877f]">{indexedDocuments.length || successCount || 1} {(indexedDocuments.length || successCount || 1) === 1 ? "source was" : "sources were"} successfully processed{successChunks ? ` into ${successChunks} searchable chunks` : " and can now be searched"}. You can now ask playground questions and verify every answer against its original source.</p><button onClick={() => router.push(`/projects/${projectId}/sources`)} className="mx-auto mt-7 flex h-11 items-center gap-2 rounded-lg bg-[var(--accent)] px-6 text-[10px] font-semibold text-[var(--ink-inverse)] hover:bg-[var(--accent-hover)]"><Sparkles className="size-4" />Open sources<ArrowRight className="size-3.5" /></button></section> : null}
    </div>
  </div>;
}
