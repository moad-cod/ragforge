...
export function WorkspaceEntry({projectId}: {projectId: string}) {
  ...
  const ready = documents.some((document) => document.status === "indexed");

  useEffect(() => {
    if (!isLoading && !isError && !ready) router.replace(`/projects/${projectId}/onboarding`);
  }, [isError, isLoading, projectId, ready, router]);
  ...
  if (isLoading || !ready) return <div className="flex h-full items-center justify-center"><div className="flex items-center gap-2 text-[10px] text-[#71847b]"><LoaderCircle className="size-4 animate-spin text-emerald-400" />Preparing your project experience…</div></div>;
