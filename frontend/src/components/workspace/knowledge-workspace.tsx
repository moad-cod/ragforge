...
import type {Document as ApiDocument, Project, RetrievalTrace} from "@/lib/types";
...
export function KnowledgeWorkspace({projectId}: {projectId: string}) {
  ...
  function toggleSelected(id: string) {
    setSelectedIds((current) => {
      const base = current.length || selectedDocumentId ? current : autoSelectedDocument ? [autoSelectedDocument.document_id] : [];
  ...
  return <div className="relative flex h-full min-w-0 overflow-hidden">
    <div className="hidden h-full shrink-0 lg:block" style={{width: leftOpen ? leftWidth : 0}}>
      {leftOpen ? <DocumentPanel projectName={projectName} documents={documents} selectedIds={effectiveSelectedIds} selectedDocumentId={activeDocument?.document_id ?? ""} onSelectDocument={(id) => {setSelectedDocumentId(id); setCitation(undefined); setRightOpen(true);}} onToggleSelected={toggleSelected} onUpload={upload} /> : null}
    </div>
    ...
    {mobileDocuments ? <div className="fixed inset-0 z-[80] lg:hidden"><button aria-label="Close documents" className="absolute inset-0 bg-black/65 backdrop-blur-sm" onClick={() => setMobileDocuments(false)} /><div className="absolute inset-y-0 left-0 w-[88%] max-w-[360px] border-r border-white/10 shadow-2xl"><DocumentPanel projectName={projectName} documents={documents} selectedIds={effectiveSelectedIds} selectedDocumentId={activeDocument?.document_id ?? ""} onSelectDocument={(id) => {setSelectedDocumentId(id); setMobileDocuments(false);}} onToggleSelected={toggleSelected} onUpload={upload} /><button onClick={() => setMobileDocuments(false)} className="absolute right-2 top-2 flex size-7 items-center justify-center rounded-lg bg-white/5"><X className="size-3.5" /></button></div></div> : null}
