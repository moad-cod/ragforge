  ...
  return <div className="relative flex h-full min-w-0 overflow-hidden">
    <main className="unified-workspace relative grid min-w-[320px] flex-1 grid-rows-[auto_minmax(0,1fr)] bg-[var(--background)]">
      ...
      <div className="absolute right-2 top-[57px] z-20 hidden gap-1 xl:flex">
        <button onClick={() => setLeftOpen((value) => !value)} title={leftOpen ? "Collapse sources" : "Open sources"} className="flex size-7 items-center justify-center rounded-lg border border-white/[0.08] bg-[var(--surface-muted)] text-[#64736d] shadow-lg hover:text-white">{leftOpen ? <PanelRightClose className="size-3.5" /> : <PanelRightOpen className="size-3.5" />}</button>
      </div>
      <button onClick={() => setMobileDocuments(true)} className="absolute bottom-14 right-3 z-30 flex h-8 items-center gap-1.5 rounded-lg border border-white/10 bg-[var(--surface-raised)] px-2.5 text-xs text-[#a9b7b0] shadow-lg xl:hidden"><FileStack className="size-3" />Sources</button>
      <button disabled={!activeDocument} onClick={() => setWorkspaceTab("document")} className="absolute bottom-24 right-3 z-30 flex h-8 items-center gap-1.5 rounded-lg border border-white/10 bg-[var(--surface-raised)] px-2.5 text-xs text-[#a9b7b0] shadow-lg disabled:hidden xl:hidden"><PanelRightOpen className="size-3" />Source</button>
    </main>
    ...
    {mobileDocuments ? <div className="fixed inset-0 z-[80] xl:hidden"><button aria-label="Close sources" className="absolute inset-0 bg-black/65 backdrop-blur-sm" onClick={() => setMobileDocuments(false)} /><div className="absolute inset-y-0 right-0 w-[88%] max-w-[360px] border-l border-white/10 shadow-2xl"><DocumentPanel projectId={projectId} projectName={projectName} documents={documents} runs={runs} chunkers={chunkers} selectedIds={effectiveSelectedIds} selectedDocumentId={activeDocument?.document_id ?? ""} onSelectDocument={(id) => {setSelectedDocumentId(id); setMobileDocuments(false);}} onOpenDocument={(id) => {openDocument(id); setMobileDocuments(false);}} onToggleSelected={toggleSelected} onUpload={upload} onUrl={addUrl} onGDrive={addGDrive} onDelete={setDeleting} onRetry={(run) => retryRun.mutate(run)} /><button onClick={() => setMobileDocuments(false)} className="absolute left-2 top-2 flex size-7 items-center justify-center rounded-lg bg-white/5"><X className="size-3.5" /></button></div></div> : null}
