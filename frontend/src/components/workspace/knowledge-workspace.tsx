...
export function KnowledgeWorkspace({projectId}: {projectId: string}) {
  ...
  const documentLabel = activeDocument?.filename?.replace(/\.(pdf|docx|xlsx|csv|txt|md|html?)$/i, "") ?? "Document";
  return <div className="relative flex h-full min-w-0 overflow-hidden">
    <main className="unified-workspace relative grid min-w-[320px] flex-1 grid-rows-[auto_minmax(0,1fr)] bg-[var(--background)]">
      <div className="workspace-tabs">
        <button type="button" data-active={workspaceTab === "assistant"} onClick={() => setWorkspaceTab("assistant")} className="workspace-tab"><MessageSquare className="size-3.5" /><span className="workspace-tab-label">Assistant</span></button>
        <button type="button" data-active={workspaceTab === "document"} onClick={() => activeDocument ? setWorkspaceTab("document") : undefined} disabled={!activeDocument} title={activeDocument?.filename ?? "Select a document"} className="workspace-tab disabled:cursor-not-allowed disabled:opacity-45"><FileText className="size-3.5" /><span className="workspace-tab-label">{activeDocument ? `Document · ${documentLabel}` : "Document"}</span></button>
      </div>
      <div className="relative min-h-0">
        ...
        <div className={workspaceTab === "document" ? "absolute inset-0" : "absolute inset-0 hidden"} aria-hidden={workspaceTab !== "document"}>
          {activeDocument ? <SourceInspector document={activeDocument} citation={citation} activeTab={tab} onTabChange={setTab} onClose={() => setWorkspaceTab("assistant")} /> : <div className="flex h-full flex-col items-center justify-center px-6 text-center"><FileText className="size-8 text-[var(--ink-faint)]" /><h2 className="mt-3 text-sm font-semibold">No document selected</h2><p className="mt-2 max-w-sm text-xs leading-5 text-[var(--ink-muted)]">Select a document in the knowledge panel, then choose Open document to inspect content, versions, retrieval traces, and metadata.</p></div>}
        </div>
      </div>
      <div className="absolute right-2 top-[57px] z-20 hidden gap-1 xl:flex">
        <button onClick={() => setLeftOpen((value) => !value)} title={leftOpen ? "Collapse documents" : "Open documents"} className="flex size-7 items-center justify-center rounded-lg border border-white/[0.08] bg-[var(--surface-muted)] text-[#64736d] shadow-lg hover:text-white">{leftOpen ? <PanelRightClose className="size-3.5" /> : <PanelRightOpen className="size-3.5" />}</button>
      </div>
      <button onClick={() => setMobileDocuments(true)} className="absolute bottom-14 right-3 z-30 flex h-8 items-center gap-1.5 rounded-lg border border-white/10 bg-[var(--surface-raised)] px-2.5 text-xs text-[#a9b7b0] shadow-lg xl:hidden"><FileStack className="size-3" />Documents</button>
      <button disabled={!activeDocument} onClick={() => setWorkspaceTab("document")} className="absolute bottom-24 right-3 z-30 flex h-8 items-center gap-1.5 rounded-lg border border-white/10 bg-[var(--surface-raised)] px-2.5 text-xs text-[#a9b7b0] shadow-lg disabled:hidden xl:hidden"><PanelRightOpen className="size-3" />Document</button>
    </main>
    ...
    {mobileDocuments ? <div className="fixed inset-0 z-[80] xl:hidden"><button aria-label="Close documents" className="absolute inset-0 bg-black/65 backdrop-blur-sm" onClick={() => setMobileDocuments(false)} /><div className="absolute inset-y-0 right-0 w-[88%] max-w-[360px] border-l border-white/10 shadow-2xl"><DocumentPanel projectId={projectId} projectName={projectName} documents={documents} runs={runs} chunkers={chunkers} selectedIds={effectiveSelectedIds} selectedDocumentId={activeDocument?.document_id ?? ""} onSelectDocument={(id) => {setSelectedDocumentId(id); setMobileDocuments(false);}} onOpenDocument={(id) => {openDocument(id); setMobileDocuments(false);}} onToggleSelected={toggleSelected} onUpload={upload} onUrl={addUrl} onGDrive={addGDrive} onDelete={setDeleting} onRetry={(run) => retryRun.mutate(run)} /><button onClick={() => setMobileDocuments(false)} className="absolute left-2 top-2 flex size-7 items-center justify-center rounded-lg bg-white/5"><X className="size-3.5" /></button></div></div> : null}
