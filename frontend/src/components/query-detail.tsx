...
export function QueryDetail({projectId, queryId}: {projectId: string; queryId: string}) {
  ...
  return <div className="mx-auto max-w-6xl space-y-6"><Link href={`/projects/${projectId}/history`} className="inline-flex items-center gap-1.5 text-[10px] text-[#71847b] hover:text-white"><ArrowLeft className="size-3" />Query history</Link><PageHeader eyebrow={project.data?.name ?? "Project"} title="Query detail" description="Inspect the persisted answer and every ranked retrieval record used to produce it." actions={item.answer ? <Button variant="secondary" size="sm" onClick={() => {void navigator.clipboard?.writeText(item.answer ?? ""); toast.success("Answer copied");}}><Copy className="size-3.5" />Copy answer</Button> : undefined} />
    <section className="rounded-xl border border-white/[0.08] bg-[#0a1511] p-5"><p className="text-[9px] font-semibold uppercase tracking-[.13em] text-[#53625b]">Question</p><h2 className="mt-2 text-base font-semibold leading-7">{item.question}</h2><div className="mt-4 flex flex-wrap gap-2">{item.provider ? <Badge tone="info">{item.provider}</Badge> : null}{item.model ? <Badge>{item.model}</Badge> : null}{item.cache_hit ? <Badge tone="success">cache hit</Badge> : <Badge>generated</Badge>}<Badge><Clock3 className="mr-1 size-3" />{formatLatency(item.latency_ms)}</Badge></div></section>
