...
export function ObservabilityDashboard({projectId}: {projectId?: string}) {
  ...
  return <div className="mx-auto max-w-7xl space-y-6"><PageHeader eyebrow={project.data?.name ?? "Control plane"} title="Observability" description="Real metrics derived from PostgreSQL document, ingestion, and query records. No estimated or synthetic values are shown." />
    ...
    <div className="grid gap-5 xl:grid-cols-2"><section className="rounded-xl border border-white/[0.08] bg-[#0a1511]"><div className="flex items-center justify-between border-b border-white/[0.08] p-4"><div><h2 className="text-sm font-semibold">Recent ingestion</h2><p className="mt-1 text-[9px] text-[#64736d]">{failedRuns} failed runs in the loaded window</p></div><Workflow className="size-4 text-[#53625b]" /></div><div className="divide-y divide-white/[0.07]">{runItems.slice(0,6).map((run) => {const targetProject = projectId ?? (run as IngestionRun & {project?: Project}).project?.project_id; return <Link key={run.ingestion_run_id} href={`/projects/${targetProject}/runs/${run.ingestion_run_id}`} className="flex items-center gap-3 p-4 hover:bg-white/[0.02]"><span className="min-w-0 flex-1"><span className="mono block truncate text-[9px] text-[#a9b7b0]">{run.ingestion_run_id}</span><span className="mt-1 block text-[8px] text-[#53625b]">{relativeTime(run.created_at)}</span></span><StatusBadge status={run.status} /></Link>;})}{!runItems.length ? <p className="p-8 text-center text-[10px] text-[#64736d]">No ingestion runs yet.</p> : null}</div></section>
      <section className="rounded-xl border border-white/[0.08] bg-[#0a1511]"><div className="flex items-center justify-between border-b border-white/[0.08] p-4"><div><h2 className="text-sm font-semibold">Recent queries</h2><p className="mt-1 text-[9px] text-[#64736d]">Durable provider and latency records</p></div><History className="size-4 text-[#53625b]" /></div><div className="divide-y divide-white/[0.07]">{queries.slice(0,6).map((query) => {const targetProject = projectId ?? (query as QueryHistoryItem & {project?: Project}).project?.project_id; return <Link key={query.query_log_id} href={`/projects/${targetProject}/history/${query.query_log_id}`} className="flex items-center gap-3 p-4 hover:bg-white/[0.02]"><span className="min-w-0 flex-1"><span className="block truncate text-[10px] font-medium">{query.question}</span><span className="mt-1 flex items-center gap-1 text-[8px] text-[#53625b]"><Clock3 className="size-2.5" />{formatLatency(query.latency_ms)} · {query.provider ?? "provider unavailable"}</span></span><StatusBadge status={query.answer ? "answered" : "failed"} /></Link>;})}{!queries.length ? <p className="p-8 text-center text-[10px] text-[#64736d]">No query history yet.</p> : null}</div></section></div>
  </div>;
}
