...
export function IngestionPipeline({run, document, projectId, compact = false}: {run: IngestionRun; document?: Document; projectId: string; compact?: boolean}) {
  ...
  return <section className="rounded-xl border border-white/[0.08] bg-[#0a1511] p-4" aria-live="polite">
    <div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0"><h3 className="truncate text-xs font-semibold">{document?.filename ?? "Document ingestion"}</h3><p className="mono mt-1 truncate text-[8px] text-[#53625b]">{run.ingestion_run_id}</p></div><StatusBadge status={run.status} /></div>
    <div className={cn("mt-4 grid gap-2", compact ? "grid-cols-3 sm:grid-cols-5" : "sm:grid-cols-3 lg:grid-cols-9")}>
      {stages.map((stage, index) => {const done = completed[index]; const active = !failed && current === index; return <div key={stage.label} className="relative flex items-center gap-2 lg:flex-col lg:text-center"><span className={cn("flex size-6 shrink-0 items-center justify-center rounded-full border", done ? "border-emerald-400 bg-emerald-400 text-[#052116]" : active ? "border-blue-400 bg-blue-400/10 text-blue-300" : failed && current === index ? "border-red-400 bg-red-400/10 text-red-300" : "border-white/10 text-[#40524a]")}>{done ? <Check className="size-3.5" /> : active ? <LoaderCircle className="size-3.5 animate-spin" /> : <Circle className="size-2.5" />}</span><span className={cn("text-[9px] leading-4", done ? "text-[#b8c5bf]" : active ? "text-blue-200" : "text-[#53625b]")}>{stage.label}</span></div>;})}
    </div>
    {!compact ? <div className="mt-4 grid gap-2 border-t border-white/[0.07] pt-3 text-[9px] text-[#64736d] sm:grid-cols-3"><span>Created {relativeTime(run.created_at)}</span><span>Started {run.started_at ? relativeTime(run.started_at) : "not reported"}</span><span>Finished {run.finished_at ? relativeTime(run.finished_at) : "not yet"}</span></div> : null}
    {failed ? <div className="mt-4 rounded-lg border border-red-400/15 bg-red-400/[0.045] p-3"><div className="flex gap-2.5"><TriangleAlert className="mt-0.5 size-4 shrink-0 text-red-300" /><div className="min-w-0 flex-1"><p className="text-[11px] font-semibold text-red-200">Document processing failed</p><p className="mt-1 text-[10px] leading-5 text-red-200/70">{run.error_message || "The pipeline stopped before indexing completed."}</p><details className="mt-2"><summary className="cursor-pointer text-[9px] text-[#a9b7b0]">Technical details</summary><div className="mono mt-2 rounded-md bg-black/20 p-2 text-[8px] leading-4 text-[#83948c]">Run: {run.ingestion_run_id}<br />DAG: {run.airflow_dag_run_id || "not assigned"}<br />Retry reuses the landed Bronze artifact and does not create a new version.</div></details><div className="mt-3 flex flex-wrap gap-2"><Button size="sm" variant="secondary" disabled={retry.isPending || run.status !== "failed"} onClick={() => retry.mutate()}><RefreshCw className={cn("size-3.5", retry.isPending && "animate-spin")} />Retry from Bronze</Button><Button size="sm" variant="ghost" onClick={() => {void navigator.clipboard?.writeText(`Run: ${run.ingestion_run_id}\nStatus: ${run.status}\nDocument: ${run.document_id}\nError: ${run.error_message || "none"}`); toast.success("Diagnostics copied");}}><Copy className="size-3.5" />Copy diagnostics</Button><Link href={`/projects/${projectId}/documents/${run.document_id}`} className="flex h-9 items-center px-3 text-[10px] text-emerald-300 hover:text-emerald-200">Open document</Link></div></div></div></div> : null}
  </section>;
}
