...
export function IngestionRunsPage({projectId}: {projectId?: string}) {
  ...
  return <div className="mx-auto max-w-7xl space-y-6"><PageHeader eyebrow={projectQuery.data?.name ?? "Control plane"} title="Ingestion runs" description="Track durable pipeline execution from upload through Bronze, Silver, Gold, and Qdrant indexing." />
    <div className="flex flex-col gap-3 rounded-xl border border-white/[0.08] bg-[#0a1511] p-3 sm:flex-row"><label className="relative flex-1"><span className="sr-only">Search ingestion runs</span><Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-[#64736d]" /><Input className="pl-9" placeholder="Search by document or run ID" value={search} onChange={(event) => setSearch(event.target.value)} /></label><select value={status} onChange={(event) => setStatus(event.target.value)} className="h-10 rounded-lg border border-white/[0.08] bg-[var(--surface-muted)] px-3 text-xs outline-none" aria-label="Filter by status"><option value="all">All statuses</option>{["landed","queued","running","silver_completed","gold_completed","indexed","failed","cancelled"].map((value) => <option key={value} value={value}>{value.replaceAll("_", " ")}</option>)}</select></div>
    {loading ? <LoadingState label="Loading ingestion runs" rows={5} /> : error ? <ErrorState title="Ingestion runs could not be loaded" description="One or more project run endpoints returned an error." onRetry={() => projectId ? void Promise.all([projectRuns.refetch(), projectDocuments.refetch()]) : void overview.refetch()} /> : filtered.length ? <div className="overflow-hidden rounded-xl border border-white/[0.08] bg-[#0a1511]"><div className="divide-y divide-white/[0.07]">{filtered.map((run) => {const document = documentMap.get(run.document_id); const targetProjectId = projectId ?? (run as IngestionRun & {project?: Project}).project?.project_id; return <Link key={run.ingestion_run_id} href={`/projects/${targetProjectId}/runs/${run.ingestion_run_id}`} className="grid gap-3 p-4 hover:bg-white/[0.02] sm:grid-cols-[minmax(220px,2fr)_1fr_1fr_auto] sm:items-center"><span className="min-w-0"><span className="block truncate text-xs font-medium">{document?.filename ?? "Unknown document"}</span><span className="mono mt-1 block truncate text-[8px] text-[#53625b]">{run.ingestion_run_id}</span></span><StatusBadge status={run.status} /><span className="text-[10px] text-[#71847b]">{relativeTime(run.created_at)}</span><span className="text-[10px] text-emerald-300">Inspect →</span></Link>;})}</div></div> : <EmptyState icon={Workflow} title={search || status !== "all" ? "No matching runs" : "No ingestion runs yet"} description={search || status !== "all" ? "Try a broader search or status filter." : "Upload a document to create a durable ingestion run."} />}
  </div>;
}
