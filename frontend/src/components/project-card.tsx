...
export function ProjectCard({project, documentCount, activeRuns, view, onRename, onDelete}: {
  ...
  return <article className={cn("group relative rounded-xl border border-white/[0.08] bg-[var(--surface)] transition hover:border-[var(--accent-border)]", view === "grid" ? "p-5" : "flex items-center gap-4 p-4")}>
    ...
    <div className={cn("min-w-0 flex-1", view === "grid" && "mt-4")}>
      ...
      <p className={cn("text-[11px] leading-5 text-[#83948c]", view === "grid" ? "mt-3 line-clamp-2 min-h-10" : "mt-1 line-clamp-1")}>An isolated RAG experiment workspace for sources, playground queries, pipeline runs, and reproducible evaluations.</p>
      <div className="mt-4 flex flex-wrap items-center gap-4 text-[10px] text-[#71847b]"><span className="flex items-center gap-1.5"><FileStack className="size-3" />{documentCount === null ? "…" : documentCount} sources</span><span className="flex items-center gap-1.5"><Activity className="size-3" />{activeRuns === null ? "…" : activeRuns ? `${activeRuns} active` : "No active runs"}</span></div>
    </div>
