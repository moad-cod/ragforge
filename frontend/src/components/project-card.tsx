...
export function ProjectCard({project, documentCount, activeRuns, view, onRename, onDelete}: {
  ...
  return <article className={cn("group relative rounded-xl border border-white/[0.08] bg-[#0a1511] transition hover:border-emerald-400/20", view === "grid" ? "p-5" : "flex items-center gap-4 p-4")}>
    <span className="flex size-10 shrink-0 items-center justify-center rounded-xl bg-emerald-400/10 text-emerald-300"><FolderKanban className="size-5" /></span>
    ...
    <div className={cn("flex items-center gap-2", view === "grid" ? "mt-5 border-t border-white/[0.07] pt-4" : "shrink-0")}>
      <Link href={`/projects/${project.project_id}/documents`} className="flex h-9 flex-1 items-center justify-center gap-2 rounded-lg bg-emerald-400/10 px-3 text-[11px] font-medium text-emerald-200 hover:bg-emerald-400/15">Open workspace<ArrowRight className="size-3.5" /></Link>
