  ...
  return <div className="mx-auto max-w-7xl space-y-6">
    ...
    <div className="grid gap-5 xl:grid-cols-2">
      <section className="rounded-xl border border-[var(--border)] bg-[var(--surface)]">
        <div className="flex items-center justify-between border-b border-white/[0.08] p-4"><div><h2 className="text-sm font-semibold">Recent pipeline runs</h2><p className="mt-1 text-[9px] text-[#64736d]">Operational state for this project</p></div><Link href={`/projects/${projectId}/pipelines`} className="text-[10px] text-[var(--accent)]">View runs</Link></div>
        <div className="divide-y divide-white/[0.07]">{runItems.slice(0, 5).map((run) => <Link key={run.ingestion_run_id} href={`/projects/${projectId}/runs/${run.ingestion_run_id}`} className="flex items-center gap-3 p-4 hover:bg-white/[0.02]"><span className="min-w-0 flex-1"><span className="mono block truncate text-[9px] text-[#a9b7b0]">{run.ingestion_run_id}</span><span className="mt-1 block text-[8px] text-[#53625b]">{relativeTime(run.created_at)}</span></span><StatusBadge status={run.status} /></Link>)}{!runItems.length ? <p className="p-8 text-center text-[10px] text-[#64736d]">No runs yet. Add sources to start ingestion.</p> : null}</div>
      </section>
      <section className="rounded-xl border border-[var(--border)] bg-[var(--surface)]">
        <div className="flex items-center justify-between border-b border-white/[0.08] p-4"><div><h2 className="text-sm font-semibold">Latest playground result</h2><p className="mt-1 text-[9px] text-[#64736d]">Question, latency, and answer status</p></div><Link href={`/projects/${projectId}/playground`} className="text-[10px] text-[var(--accent)]">Open playground</Link></div>
        {lastQuery ? <Link href={`/projects/${projectId}/history/${lastQuery.query_log_id}`} className="block p-4 hover:bg-white/[0.02]"><p className="line-clamp-2 text-xs font-medium leading-5">{lastQuery.question}</p><p className="mt-2 line-clamp-2 text-[10px] leading-5 text-[#71847b]">{lastQuery.answer ?? "No answer was persisted."}</p><div className="mt-3 flex flex-wrap gap-2 text-[9px] text-[#53625b]"><span>{formatLatency(lastQuery.latency_ms)}</span><span>{lastQuery.cache_hit ? "cached" : "generated"}</span><span>{lastQuery.model ?? "model unavailable"}</span></div></Link> : <p className="p-8 text-center text-[10px] text-[#64736d]">No playground queries yet. Ask a question after indexing sources.</p>}
      </section>
    </div>
  </div>;
}
