...
export default function ProjectSettingsPage({params}: {params: Promise<{projectId: string}>}) {
  ...
  return <div className="mx-auto max-w-4xl space-y-6"><PageHeader eyebrow={project.data.name} title="Project settings" description="Update settings supported by the current project API. Collection identity remains immutable to preserve indexed data." />
    <section className="rounded-xl border border-white/[0.08] bg-[var(--surface)] p-5"><div className="flex items-center gap-3"><span className="flex size-9 items-center justify-center rounded-lg bg-[var(--accent-soft)] text-[var(--accent)]"><Settings className="size-4" /></span><div><h2 className="text-sm font-semibold">General</h2><p className="mt-0.5 text-[9px] text-[#77716a]">The backend currently supports project-name updates.</p></div></div><label className="mt-5 block"><span className="mb-2 block text-xs font-medium">Project name</span><Input value={effectiveName} onChange={(event) => setName(event.target.value)} /></label><div className="mt-5 flex justify-end"><Button disabled={rename.isPending || effectiveName.trim().length < 2 || effectiveName === project.data.name} onClick={() => rename.mutate()}>{rename.isPending ? <LoaderCircle className="size-4 animate-spin" /> : <Save className="size-4" />}Save name</Button></div></section>
    <section className="rounded-xl border border-white/[0.08] bg-[var(--surface)] p-5"><div className="flex items-center gap-3"><Database className="size-4 text-[#77716a]" /><div><h2 className="text-sm font-semibold">Storage identity</h2><p className="mt-0.5 text-[9px] text-[#77716a]">Read-only values assigned by the backend.</p></div></div><dl className="mt-5 space-y-3 text-[10px]"><div className="grid gap-1 sm:grid-cols-[150px_1fr]"><dt className="text-[#77716a]">Project ID</dt><dd className="mono break-all text-[#b7b0a7]">{project.data.project_id}</dd></div><div className="grid gap-1 sm:grid-cols-[150px_1fr]"><dt className="text-[#77716a]">Qdrant collection</dt><dd className="mono break-all text-[#b7b0a7]">{project.data.qdrant_collection}</dd></div></dl></section>
    <section className="rounded-xl border border-red-400/15 bg-red-400/[0.025] p-5"><h2 className="text-sm font-semibold text-red-200">Danger zone</h2><p className="mt-2 text-[10px] leading-5 text-[#aaa39a]">Deleting this project removes {documents.data?.length ?? "all"} sources and its Qdrant collections. Playground history will no longer be accessible through the project.</p><Button className="mt-4" variant="danger" onClick={() => setConfirm(true)}><Trash2 className="size-4" />Delete project</Button></section>
    <ConfirmDeleteDialog open={confirm} onClose={() => setConfirm(false)} name={project.data.name} title={`Delete ${project.data.name}?`} consequences={`${documents.data?.length ?? "All"} sources, indexed chunks, and accessible playground history will be affected.`} isPending={remove.isPending} onConfirm={() => remove.mutate()} />
  </div>;
}
