...
export default function DocumentsPage() {
  ...
  return <div className="mx-auto max-w-7xl space-y-6"><PageHeader eyebrow="Knowledge" title="Documents" description="Browse source documents across projects. Open a document to inspect its immutable version lineage and ingestion runs." />
    ...
    {overview.pending ? <LoadingState label="Loading documents" rows={5} /> : overview.error ? <ErrorState title="Documents could not be loaded" description="At least one owned project returned an error while loading its documents." onRetry={() => void overview.refetch()} /> : filtered.length ? <DocumentList items={filtered} onDelete={setSelected} /> : <EmptyState icon={FileStack} title={search || status !== "all" ? "No matching documents" : "No documents yet"} description={search || status !== "all" ? "Try a different filename or status." : "Open a project and upload a source to begin."} />}
