...
export default function OrganizationPage() {
  ...
  return <div className="mx-auto max-w-5xl space-y-6"><PageHeader eyebrow="Account" title="Organization settings" description="Create and rename organizations, then choose your active organization from the application top bar." actions={<Button onClick={() => open("new")}><Plus className="size-4" />New organization</Button>} />
    {organizations.isLoading ? <LoadingState label="Loading organizations" rows={3} /> : organizations.isError ? <ErrorState title="Organizations could not be loaded" description="The authenticated organization API returned an error." onRetry={() => void organizations.refetch()} /> : organizations.data?.length ? <div className="overflow-hidden rounded-xl border border-white/[0.08] bg-[var(--surface)]">{organizations.data.map((organization) => <article key={organization.organization_id} className="flex flex-col gap-4 border-b border-white/[0.07] p-4 last:border-0 sm:flex-row sm:items-center"><span className="flex size-10 items-center justify-center rounded-xl bg-[var(--accent-soft)] text-[var(--accent)]"><Building2 className="size-5" /></span><div className="min-w-0 flex-1"><h2 className="truncate text-sm font-semibold">{organization.name}</h2><p className="mono mt-1 truncate text-[8px] text-[#5f5952]">{organization.organization_id}</p><p className="mt-1 text-[9px] text-[#77716a]">Updated {relativeTime(organization.updated_at)}{user.data?.organization_id === organization.organization_id ? " · Active" : ""}</p></div><div className="flex gap-1"><Button variant="ghost" size="sm" onClick={() => open(organization)}><Pencil className="size-3.5" />Rename</Button><Button variant="ghost" size="sm" className="text-red-300" onClick={() => setDeleting(organization)}><Trash2 className="size-3.5" />Delete</Button></div></article>)}</div> : <EmptyState icon={Building2} title="No organizations yet" description="Projects can remain in your personal workspace, or you can create an organization." action="Create organization" onAction={() => open("new")} />}
